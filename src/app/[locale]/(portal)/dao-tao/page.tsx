...
export default async function HomePage({
  ...
  for (const program of programs) {
    ...
    programCountBySchool.set(
      program.schoolId,
      (programCountBySchool.get(program.schoolId) ?? 0) + 1,
    );
  }
  const schoolName = new Map(
    schools.map((s) => [s.id, t(s.shortName ?? s.name, locale)]),
  );
  ...
  const fields = categories
    .map((category) => {
      const inField = programs.filter(
        (program) => program.categoryId === category.id,
      );
      return {
        slug: category.slug,
        ...
        programs: inField.slice(0, 6).map((program) => ({
          ...
          school: program.schoolId
            ? (schoolName.get(program.schoolId) ?? "")
            : "",
        })),
      };
    })
  ...
  const categoryName = new Map(
    categories.map((c) => [c.id, t(c.name, locale)]),
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      label: pick(
        {
          vi: "ngành nghề đã đăng ký hoạt động",
  ...
  const pathway = [
    {
      ...
      body: pick(
        {
          vi: "Trang bị kiến thức chuyên môn và ngoại ngữ theo chuẩn nghề nghiệp.",
          en: "Build professional knowledge and foreign-language skills to occupational standards.",
          de: "Fachwissen und Fremdsprachen nach beruflichen Standards aufbauen.",
          ja: "職業基準に沿った専門知識と外国語力を身につけます。",
          ko: "직업 표준에 맞춘 전문 지식과 외국어 실력을 갖춥니다.",
          "zh-TW": "依職業標準培養專業知識與外語能力。",
        },
        locale,
      ),
    },
      ...
      body: pick(
        {
          ...
          en: "Develop hands-on skills in the school's workshops and laboratories.",
          de: "Praktische Fertigkeiten in Werkstätten und Laboren entwickeln.",
          ja: "学校の実習工場と実験室で職業技能を磨きます。",
          ko: "학교의 실습 공장과 실험실에서 직업 기능을 익힙니다.",
          "zh-TW": "在學校的實習工場與實驗室中鍛鍊職業技能。",
        },
        locale,
      ),
    },
    {
      ...
      body: pick(
        {
          vi: "Làm việc thực tế tại doanh nghiệp đối tác trong và ngoài nước.",
          en: "Work in real conditions at partner employers in Vietnam and abroad.",
          de: "Im Betrieb arbeiten – bei Partnern in Vietnam und im Ausland.",
          ja: "国内外の提携企業で実際の現場に立ちます。",
          ko: "국내외 협력 기업에서 실제 현장을 경험합니다.",
          "zh-TW": "在國內外合作企業的真實現場工作。",
        },
        locale,
      ),
    },
    {
      ...
      body: pick(
        {
          vi: "Ứng tuyển và phát triển nghề nghiệp lâu dài với hỗ trợ của nhà trường.",
          en: "Apply and build a long-term career with the school's support.",
          de: "Bewerben und mit Unterstützung der Schule langfristig Karriere machen.",
          ja: "学校の支援を受けて応募し、長く続く職業人生を築きます。",
          ko: "학교의 지원을 받아 지원하고 오래가는 경력을 쌓습니다.",
          "zh-TW": "在學校協助下應徵求職，發展長遠的職涯。",
        },
        locale,
      ),
    },
  ...
  const reasons = [
    {
      claim: pick(
        {
          vi: "Gắn với giấy phép, không phải lời hứa",
          en: "Tied to licences, not promises",
          de: "An Zulassungen gebunden, nicht an Versprechen",
          ja: "約束ではなく、認可に裏づけられて",
          ko: "약속이 아니라 인가에 근거합니다",
          "zh-TW": "依據立案文件，而非口頭承諾",
        },
        locale,
      ),
      body: pick(
        {
          vi: "Mỗi ngành trên website đều kèm mã ngành/nghề và quy mô tuyển sinh trích từ giấy chứng nhận đăng ký hoạt động giáo dục nghề nghiệp, có số hiệu và ngày cấp.",
          en: "Every programme here carries the occupation code and intake quota transcribed from the vocational-education registration certificate, with its number and date of issue.",
          de: "Jedes Programm nennt den amtlichen Berufscode und die Aufnahmekapazität aus dem Zulassungsbescheid – mit Nummer und Datum.",
          ja: "本サイトの各課程には、技能教育の立案証明書から書き写した職種コードと募集定員が、証明書番号と交付日とともに添えられています。",
          ko: "이 사이트의 모든 과정에는 직업교육 인가증에서 옮겨 적은 직종 코드와 모집 정원이 인가증 번호 및 발급일과 함께 표시됩니다.",
          "zh-TW":
            "本網站的每一個課程都附有自技職教育立案證明書抄錄的職類代碼與招生名額，並註明證書字號與核發日期。",
        },
        locale,
      ),
      evidence: pick(
        {
          vi: `${programs.length} ngành đã đăng ký hoạt động, trích từ giấy chứng nhận của ${schools.length} trường`,
          en: `${programs.length} registered occupations, transcribed from the certificates of ${schools.length} schools`,
          de: `${programs.length} registrierte Berufsprofile aus den Bescheiden von ${schools.length} Schulen`,
        },
        locale,
      ),
      image: {
    ...
    },
    {
      claim: pick(
        {
          vi: "Thực hành chiếm phần lớn thời lượng",
          en: "Practice takes most of the time",
          de: "Praxis nimmt den größten Teil ein",
          ja: "時間の大半は実習に",
          ko: "수업 시간의 대부분은 실습",
          "zh-TW": "課程時數多半用於實作",
        },
      ...
      body: pick(
        {
          vi: "Các trường kỹ thuật trong hệ thống bố trí khoảng 70% thời lượng cho thực hành tại xưởng và tại doanh nghiệp.",
          en: "The technical schools in the system devote around 70% of training time to workshop and workplace practice.",
          de: "Die technischen Schulen widmen rund 70 % der Ausbildungszeit der Praxis.",
          ja: "系列の工業系学校では、訓練時間のおよそ 70% を工場および企業での実習に充てています。",
          ko: "체계 안의 공업계 학교들은 훈련 시간의 약 70%를 공장과 기업 실습에 배정합니다.",
          "zh-TW": "體系內的技術學校將約 70% 的訓練時數安排於工場與企業實作。",
        },
        locale,
      ),
      evidence: pick(
        {
          vi: "Hồ sơ năng lực Việt Đức Group, phần chương trình đào tạo",
          en: "Viet Duc Group capability profile, training chapter",
          de: "Leistungsprofil der Viet Duc Group, Kapitel Ausbildung",
          ja: "Viet Duc Group 会社案内、教育課程の章",
          ko: "Viet Duc Group 역량 소개서, 교육 과정 부분",
          "zh-TW": "Viet Duc Group 能力簡介，培訓課程章節",
        },
        locale,
      ),
      image: {
        ...
        alt: pick(
          {
            ...
            ja: "NIBELC ネットワーク企業の生産ラインで実習する学生",
            ko: "NIBELC 네트워크 기업의 생산 라인에서 실습하는 학생",
            "zh-TW": "學員在 NIBELC 網絡企業的生產線上實作",
          },
          locale,
        ),
      },
    },
    {
      claim: pick(
        {
          vi: "Một cửa ngõ sang Đức và châu Âu",
          en: "A route into Germany and Europe",
          de: "Ein Weg nach Deutschland und Europa",
          ja: "ドイツとヨーロッパへの入口",
          ko: "독일과 유럽으로 가는 관문",
          "zh-TW": "通往德國與歐洲的門戶",
        },
      ...
      body: pick(
        {
          vi: "Viện Đào tạo và Giáo dục ITW Berlin chuyển giao chương trình, phương pháp và đào tạo tiếng Đức ngay tại Việt Nam.",
          en: "The ITW Berlin institute transfers programmes, teaching methods and German-language training directly into Vietnam.",
          de: "Das itw Berlin überträgt Programme, Methoden und Deutschunterricht direkt nach Vietnam.",
          ja: "ITW ベルリン教育訓練研究所が、教育課程・指導法・ドイツ語教育をベトナム国内に移転しています。",
          ko: "ITW 베를린 교육훈련원이 교육 과정과 교수법, 독일어 교육을 베트남 현지로 이전합니다.",
          "zh-TW":
            "ITW 柏林教育訓練學院將課程、教學方法與德語教學直接移轉至越南。",
        },
        locale,
      ),
      evidence: pick(
        {
          vi: "ITW Berlin – thành viên của hệ thống, đặt tại CHLB Đức",
          en: "ITW Berlin - member of the network, based in Germany",
          de: "ITW Berlin – Mitglied des Verbunds, Sitz in Deutschland",
          ja: "ITW ベルリン — ドイツ連邦共和国に置かれた系列校",
          ko: "ITW 베를린 — 독일 연방공화국에 있는 회원 기관",
          "zh-TW": "ITW 柏林 — 設於德意志聯邦共和國的體系成員",
        },
        locale,
      ),
      image: {
        ...
        alt: pick(
          {
            ...
            ja: "NIBELC ネットワークのヨーロッパ提携先との面談",
            ko: "NIBELC 네트워크의 유럽 협력사와의 만남",
            "zh-TW": "與 NIBELC 網絡中的歐洲夥伴會面",
          },
          locale,
        ),
      },
    },
    {
      claim: pick(
        {
          vi: `${schools.length} trường, một hệ thống`,
          en: `${schools.length} schools, one system`,
          de: `${schools.length} Schulen, ein Verbund`,
          ja: `${schools.length}つの学校、ひとつの体系`,
          ko: `${schools.length}개 학교, 하나의 체계`,
          "zh-TW": `${schools.length} 所學校，一個體系`,
        },
        locale,
      ),
      body: pick(
        {
          vi: "Từ Đà Nẵng, Ninh Bình, Vũng Tàu, Đồng Nai đến Quảng Trị và Berlin – người học chọn ngành trước, chọn nơi học sau.",
          en: "From Da Nang, Ninh Binh, Vung Tau and Dong Nai to Quang Tri and Berlin: choose the field first, the campus second.",
          de: "Von Da Nang, Ninh Binh, Vung Tau und Dong Nai bis Quang Tri und Berlin – erst das Fach, dann der Standort.",
          ja: "ダナン、ニンビン、ブンタウ、ドンナイから、クアンチ、そしてベルリンまで。まず分野を選び、学ぶ場所はその次に。",
          ko: "다낭, 닌빈, 붕따우, 동나이에서 꽝찌와 베를린까지. 먼저 분야를 고르고, 배울 곳은 그다음입니다.",
          "zh-TW":
            "從峴港、寧平、頭頓、同奈，到廣治與柏林——先選領域，再選校區。",
        },
        locale,
      ),
      evidence: pick(
        {
          vi: `${schools.length} trường thành viên, hồ sơ từng trường công bố trên trang Hệ thống trường`,
          en: `${schools.length} member schools, each with its own record on the network page`,
          de: `${schools.length} Mitgliedsschulen, jede mit eigenem Profil auf der Verbundseite`,
        },
        locale,
      ),
      image: {
        ...
        alt: pick(
          {
            ...
            de: "Finalabend des Talentwettbewerbs",
            ja: "学生タレント・エレガンスコンテスト決勝の夜",
            ko: "학생 재능·품위 선발대회 결승전의 밤",
            "zh-TW": "學生才藝與儀態競賽決賽之夜",
          },
          locale,
        ),
      ...
      <section className={styles.hero}>
        <div className={styles.heroMedia}>
          ...
          <HeroPicture
            ...
            tall={{
              src: "/media/hero/vdg-hq-portrait.webp",
              width: 992,
              height: 1586,
            }}
            alt={pick(
          ...
          <HeroVideo
            src={HERO_VIDEO}
            poster="/media/hero/vdg-hq-portrait.webp"
        ...
        <div className="shell">
          <div className={styles.heroText}>
            ...
            <h1 className={styles.heroTitle}>
              <RiseText
                text={dict.brand.motto.split("–")[0].trim()}
                delay={180}
              />
              <RiseText
                as="em"
                text={dict.brand.motto.split("–")[1]?.trim() ?? ""}
                delay={520}
              />
            </h1>
            <p className={styles.heroLead}>{dict.home.heroLead}</p>
            <div className={styles.heroActions}>
              <ButtonLink href={path("/dao-tao/chuong-trinh")}>
                {dict.nav.explorer}
              </ButtonLink>
      ...
      <div className={`shell ${styles.finderWrap}`}>
        <ProgramFinder
          ...
          schools={schools.map((sc) => ({
            slug: sc.slug,
            label: t(sc.shortName ?? sc.name, locale),
          }))}
        />
      </div>
          ...
          <div className={styles.aboutGrid}>
            <div data-reveal suppressHydrationWarning>
                ...
                lead={pick(
                  {
                    ...
                    "zh-TW":
                      "Viet Duc Group 是涵蓋職業訓練、專科、中級課程與國際合作的多層級、多領域教育體系，在越南與德意志聯邦共和國設有成員學校。",
                  },
                  locale,
                )}
              />
              ...
              <p className={styles.aboutLink}>
                <ArrowLink href={path("/gioi-thieu")}>
                ...
                {[
                  {
                    ...
                    text: pick(
                      {
                        vi: "Khởi đầu với các cơ sở đào tạo nghề chất lượng cao.",
                        en: "Established the first colleges and vocational schools.",
                        de: "Gründung der ersten Colleges und Berufsschulen.",
                        ja: "質の高い職業訓練機関の設立から始まりました。",
                        ko: "수준 높은 직업훈련 기관의 설립으로 출발했습니다.",
                        "zh-TW": "從設立優質的職業訓練機構起步。",
                      },
                      locale,
                    ),
                  },
                  {
                    ...
                    text: pick(
                      {
                        vi: "Mở rộng hệ thống, đa dạng ngành đào tạo.",
                        en: "Opened the system and widened the range of programmes.",
                        de: "Ausbau des Verbunds und Erweiterung der Programme.",
                        ja: "体系を広げ、教育分野を多様にしました。",
                        ko: "체계를 넓히고 교육 분야를 다양하게 했습니다.",
                        "zh-TW": "擴展體系，豐富培訓領域。",
                      },
                      locale,
                    ),
                  },
                  {
                    ...
                    text: pick(
                      {
                        vi: "Hợp tác quốc tế, nâng tầm chất lượng đào tạo.",
                        en: "Expanded international partnerships and upgraded quality.",
                        de: "Ausbau internationaler Partnerschaften und der Qualität.",
                        ja: "国際連携を進め、教育の質を高めました。",
                        ko: "국제 협력을 넓히고 교육의 질을 높였습니다.",
                        "zh-TW": "拓展國際合作，提升培訓品質。",
                      },
                      locale,
                    ),
                  },
                  {
                    ...
                    text: pick(
                      {
                        vi: "Phát triển bền vững trong lĩnh vực giáo dục nghề nghiệp.",
                        en: "Growing sustainably in vocational education.",
                        de: "Nachhaltiges Wachstum in der beruflichen Bildung.",
                        ja: "職業教育の分野で着実に歩みを進めています。",
                        ko: "직업교육 분야에서 꾸준히 성장하고 있습니다.",
                        "zh-TW": "在技職教育領域穩健成長。",
                      },
                      locale,
                    ),
      ...
      <section className={`section ${styles.schoolsSection}`}>
        <div className="shell">
          <SectionHeading
            ...
            title={dict.home.schoolsTitle.replace(
              "{n}",
      ...
      <section className={`section ${styles.bandBeige}`}>
        <div className="shell">
          <SectionHeading
            ...
            action={
              <ArrowLink href={path("/dao-tao/chuong-trinh")}>
                {dict.common.viewAll}
              </ArrowLink>
            }
          />
          ...
          <ProgramCards
            ...
            programs={featured.map((program) => ({
              ...
              level: levelLabel(program.level, locale),
              code: program.officialCode ?? null,
              cover: program.schoolId
                ? (schoolCover.get(program.schoolId) ?? null)
                : null,
              crest: program.schoolId
                ? (schoolCrest.get(program.schoolId) ?? null)
                : null,
            }))}
          />
        </div>
      ...
      <section className={`section ${styles.whySection}`}>
        <div className="shell">
          ...
          <div className={styles.whyAction}>
            <ButtonLink href={path("/dao-tao/dang-ky-tu-van")}>
              {dict.nav.apply}
            </ButtonLink>
          </div>
        </div>
          ...
          <div className="shell">
            ...
            <div className={styles.postGrid}>
              {posts.map((post) => (
                ...
                >
                  <h3>
                    <Link href={path(`/tin-tuc/${post.slug}`)}>
                      {t(post.title, locale)}
                    </Link>
                  </h3>
